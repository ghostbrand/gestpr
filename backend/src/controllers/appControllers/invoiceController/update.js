...
const update = async (req, res) => {
  ...
  items.forEach((item) => {
    if (item.serviceItem === '' || item.serviceItem == null) {
  ...
  try {
    const populated = await Model.findOne({ _id: req.params.id, removed: false })
      .populate('createdBy', 'name')
      .exec();
    const targetLocation = `src/public/download/invoice/${body['pdf']}`;
    ...
    console.error('[invoice/update] PDF generation failed:', pdfErr?.message || pdfErr);
  }
