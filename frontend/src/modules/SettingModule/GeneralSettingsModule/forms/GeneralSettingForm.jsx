...
export default function GeneralSettingForm() {
  ...
  return (
    <div>
      ...
      <Form.Item
        label={translate('idurar_app_email')}
        name="idurar_app_email"
        rules={[{ type: 'email' }]}
        extra="Endereço remetente (From) para recuperação de palavra-passe e notificações."
      >
        <Input placeholder="ex.: noreply@crisefama.pt" />
      </Form.Item>
      <Form.Item
        label={translate('idurar_base_url')}
        name="idurar_base_url"
        rules={[{ required: true, message: 'Indique o URL público da aplicação' }]}
        extra="Usado nos links dos e-mails (ex.: http://localhost:5173 ou o domínio em produção)."
      >
