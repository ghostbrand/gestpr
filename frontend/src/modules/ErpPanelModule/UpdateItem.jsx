...
import { sumInvoiceLinesSubtotal } from '@/utils/invoiceLineTotals';
...
export default function UpdateItem({ config, UpdateForm }) {
  ...
  const handelValuesChange = (changedValues, values) => {
    ...
    if (items) {
      setSubTotal(sumInvoiceLinesSubtotal(items));
    }
  };
    ...
    if (fieldsValue) {
      ...
      if (fieldsValue.items) {
        ...
        fieldsValue.items.map((item) => {
          const { quantity, price, itemName, description, serviceItem } = item;
          const q =
            quantity === undefined || quantity === null || quantity === '' ? 1 : quantity;
          const total = Number(q) * Number(price);
          newList.push({
            total,
            quantity: q,
            price,
  ...
  useEffect(() => {
    if (current) {
      ...
      if (entity === 'serviceitem') {
        formData.itemKind = formData.itemKind || 'service';
        formData.stockQuantity = formData.stockQuantity ?? 0;
        formData.minStockLevel = formData.minStockLevel ?? 0;
        formData.unit = formData.unit || 'un';
      }
